...
public class GameScreenAshley extends AbstractGameScreen implements CollisionSystem.CollisionListener, ScoreSystem.ScoreListener {
    ...
    public void render(float delta) {
        Gdx.gl.glClearColor(1.2f, 0.2f, 0.2f, 1);
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);

        if (Gdx.input.isKeyPressed(Input.Keys.UP))
            ComponentMappers.transformMapper.get(cameraEntity).position.add(0, CAMERA_SPEED * delta);
        if (Gdx.input.isKeyPressed(Input.Keys.DOWN))
            ComponentMappers.transformMapper.get(cameraEntity).position.add(0, -CAMERA_SPEED * delta);
        if (Gdx.input.isKeyPressed(Input.Keys.LEFT))
            ComponentMappers.transformMapper.get(cameraEntity).position.add(-CAMERA_SPEED * delta, 0);
